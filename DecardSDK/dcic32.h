...
__int16  __stdcall  IC_CpuApduRespon(HANDLE idComDev,unsigned char slen,unsigned char * sendbuffer,unsigned char *rlen,unsigned char * databuffer);
	...
	*/
	...
	__int16 __stdcall IC_Command_Keypad(HANDLE idComDev,unsigned int slen,unsigned char *sbuff, unsigned int *rlen,unsigned char *rbuff);
	__int16 __stdcall IC_KeypadOpen(HANDLE icdev, int number, char *version);
	__int16 __stdcall IC_KeypadClose(HANDLE icdev, int number);
	__int16 __stdcall IC_KeypadLoadKey(HANDLE icdev, int number, int set_index, int sub_index, int type, int mode, const unsigned char *key_data, int key_len, int flag, unsigned char *ex_data, int *ex_len);
	__int16 __stdcall IC_KeypadClearKey(HANDLE icdev, int number, int set_index, int sub_index);
	__int16 __stdcall IC_KeypadGetKeyType(HANDLE icdev, int number, int set_index, int sub_index, int *type);
	__int16 __stdcall IC_KeypadAlgorithm(HANDLE icdev, int number, int set_index, int sub_index, int flag, int mode, const unsigned char *in_data, int in_len, unsigned char *out_data, int *out_len);
	__int16 __stdcall IC_KeypadStartInput(HANDLE icdev, int number, int mode, int set_index, int sub_index, const unsigned char *in_data, int in_len, int min_len, int max_len, int auto_end, int enable_beep, int time_s, unsigned char *out_data, int *out_len);
	__int16 __stdcall IC_KeypadGetKeyValue(HANDLE icdev, int number, unsigned char *value);
	__int16 __stdcall IC_KeypadExitAndGetInput(HANDLE icdev, int number, int mode, unsigned char *out_data, int *out_len);
	__int16 __stdcall IC_KeypadSetKeyValue(HANDLE icdev, int number, unsigned char keyvalue);

	__int16 __stdcall IC_SetMagMode(HANDLE idComDev,unsigned char mode);

	__int16 __stdcall IC_Reader_PowerOn(HANDLE ReaderHandle, unsigned char ICC_Slot_No, unsigned char *Response);
	__int16 __stdcall IC_Reader_PowerOff(HANDLE ReaderHandle, unsigned char ICC_Slot_No);
	__int16 __stdcall IC_Reader_GetStatus(HANDLE ReaderHandle, unsigned char ICC_Slot_No);
	__int16 __stdcall IC_Reader_Application(HANDLE idComDev, unsigned char ICC_Slot_No,__int16 slen, unsigned char * sendbuffer, __int16 *rlen, unsigned char * databuffer);

	__int16 __stdcall SD_SendGetPos(HANDLE idComDev,unsigned char pplen,unsigned char * ppdata,unsigned char * rlen,unsigned char * rrdata,unsigned char timeout,unsigned char down);
	__int16 __stdcall SD_GetVer(HANDLE idComDev, char *VerStr);
  __int16  __stdcall  SD_ReadSerialNumber(HANDLE idComDev, char *serial_number);
	__int16 __stdcall SD_Scan2DBarcodeStart(HANDLE idComDev, unsigned char mode);
	__int16 __stdcall SD_Scan2DBarcodeGetData(HANDLE idComDev, int *rlen, unsigned char *rdata);
	__int16 __stdcall SD_Scan2DBarcodeExit(HANDLE idComDev);

	__int16 __stdcall IC_RecoveryCounter_4406(HANDLE idComDev);
	__int16 __stdcall IC_Internal_Cmd(HANDLE idComDev, unsigned char cmdtype,__int16 slen, unsigned char * sendbuffer, __int16 *rlen, unsigned char * databuffer);

	void __stdcall IC_LibVer(char *ver);

  __int16 __stdcall IC_GetDeviceUid(HANDLE idComDev, char *uid);
  __int16 __stdcall IC_GetDeviceTradeNumber(HANDLE idComDev, char *number);
  __int16 __stdcall IC_GetDeviceTerminalNumber(HANDLE idComDev, char *number);
   ...
   * @param[in] charset 保留。
   * @param[in] text UNICODE（Little-Endian）编码的文本字符串。
   * @return <0表示失败，==0表示成功。
   */
  __int16 __stdcall IC_LcdDisplayText(HANDLE icdev, unsigned char line, unsigned char offset, unsigned char mode, unsigned char charset, const unsigned char *text);
   ...
   * @return <0表示失败，==0表示成功。
   */
  __int16 __stdcall IC_LcdDisplayTextLine(HANDLE icdev, unsigned char line, unsigned char mode, unsigned char charset, const unsigned char *text);

  __int16 __stdcall IC_GetInputAmountEn(HANDLE idComDev, unsigned char ctime, unsigned char *rlen, unsigned char *cpass);

  __int16 __stdcall IC_GetIdSnr(HANDLE icdev, unsigned char *_Data);
  __int16 __stdcall IC_GetIdSnr_hex(HANDLE icdev, char *_Data);
